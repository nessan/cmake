...
Invoking that target will create a zipped archive file containing the arguments that were passed to the function.
...
The archive uses the `CMake` native `tar` command.
...
For that reason, `FetchContent` supports another mode of operation where it can download just enough from the `ExternalLib` repo to make it operational for the library's consumer.
...
Finally, you will typically include the `LICENSE` file, which is not an operational necessity but generally a tiny file and best kept.

## Notes

1. Once you have set up the `archive` target, you probably want to add a job to your CI/CD pipeline that invokes `cmake` to build it automatically.
...
1. `CMake` comes with `CPack` that can do something similar.
