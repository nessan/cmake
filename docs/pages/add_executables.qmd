...
To use this function, we expect each file in the directory to compile into a separate executable.

For example, if the directory has a file `Foo.cpp` the function will create a target `Foo` that compiles `Foo.cpp` and links the object file to any trailing arguments which generally are libraries.

Moreover, if the source files in the directory follow a simple sequential naming scheme, the function will also add some convenience targets that build whole _groups_ of executables at once.
...
Library developers will often develop an extensive set of simple command line tools that exercise various features of their library either as demos or as tests.
